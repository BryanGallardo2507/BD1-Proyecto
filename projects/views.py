    ...
    def get(self, request, id=0):
        if id > 0:
            ...
            if len(usuario_data) > 0:
                ...
                accesosA = list(accesos.objects.filter(usuario = usuario.get('id')).values())
                generoA = list(genero.objects.filter(tipo_genero = usuario.get('id')).values())
                usuario['id_acceso_id'] = accesosA
...
class ArchivosView(View):
    ...
    def get(self, request, id=0):
        if id > 0:
            ...
            archivo_data = list(archivos.objects.values('id', 'nombre_archivo', 'fecha_guardado', 'tamanio', 'id_carpeta_id', 'id_tipo_archivo_id', 'id_usuario_id'))
            if len(archivo_data) > 0:
